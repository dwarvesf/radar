...
## Status of this Memo
This document is missing a detail description of [The most important feature]() in [section 3]() and reference.

## Table of Contents
1. Introduction
2. How does blockchain works? 
3. What makes a blockchain system secure?  
    a. The most important features  
    b. Cryptography in blockchain security  
      1. Cryptography techniques
      2. Application of cryptography in blockchain
...
The first thing that is mentioned in the name of this platform is **block**. Combine this with the concept introduced in [section 1]() that blockchain is not so different from a regular database, we can imagine that Blockchain uses block to store pieces of information. But it is not only one block for storing all data, a lot of blocks is generated and used in the system. Each block is time-stamped and connected to the one generated before it generated time. So a chain of blocks is created, hence the term **blockchain**.
...
### 3. What makes a blockchain system secure? 
Description in [section 2]() is a very simple view about blockchain to help everybody without technical knowledge to get an overview of the blockchain system. In the real-life, blockchain is a complex platform with a variety of properties, theories, algorithms, and mechanisms. A lot of things contribute to building blockchain security that includes advanced cryptographic techniques and mathematical models of behavior and decision-making and this is far from a simple subject. Therefore, this is very important to understand a few basic concepts and mechanisms that make blockchain have robust protection.
...
To get data security, the blockchain system uses a lot of [cryptography algorithms](). [hash function](), mentioned above, are the most popular techniques that are used in this context. 
...
Blockchain is also use hashing to get consensus in validating transactions. Bitcoin is the best example in this context, using protocols [Proof of Work](), Bitcoin applies hash function [SHA-256]() that takes information input and generates a hash string with 256 bits length or 64 characters long.

Another protection what cryptography provides for blockchain is assurance for the safety of the wallets where cryptocurrency located. [Public-Key Cryptography]() is used here to encrypt and decrypt payment information when sending and receiving. Private key is also used to generate a digital signature for transactions to get authenticate ownership of the coins that are being sent. 
...
To understand clearly the above properties of PoW and the way that they interact with each other to get security, let's take [Bitcoin (BTC)]() as an example. 
...
#### 4b. Proof of Stake
To get enough computational power, Proof of Work wasted a lot of resources all over the world. So it is said to be unsustainable in the future. A system, was designed to solve some of the inefficiencies and emerging problems that commonly arise on [PoW-based]() blockchains, is called Proof of Stake (PoS).
